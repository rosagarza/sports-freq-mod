...
//  FrequencySignal
//
//  Created by Rosa Garza on 1/22/20.
...
class SecViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        AKLog("Settings.....")
        //let recordingFormat = AudioKit.engine.inputNode.inputFormat(forBus: 0).sampleRate
        AKLog("Here.....")
        mic = AKMicrophone()
        AKLog("Mic")
...
//            silence = AKBooster(tracker, gain: 0)
            ...
            AKLog("AudioKit Started")
                   setupPlot()
            AKLog("We set up the plot")
                   Timer.scheduledTimer(timeInterval: 0.1,
                                        target: self,
                                        selector: #selector(SecViewController.updateUI),
                                        userInfo: nil,
                                        repeats: true)
        }
